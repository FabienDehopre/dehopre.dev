...
@Component({
  ...
  template: `
    <button
      ...
      (click)="setTheme.emit(otherTheme())"
    >
      <!-- Sun Icon -->
      <svg
        aria-hidden="true"
      ...
      <svg
        aria-hidden="true"
        class="
          hidden h-6 w-6 fill-zinc-700 stroke-zinc-500 transition
...
export class ThemeToggle {
  protected readonly otherTheme = computed(() => this.theme() === 'dark' ? 'light' : 'dark');
  protected readonly buttonAriaLabel = computed(() => `Switch to ${this.otherTheme()} theme`);
